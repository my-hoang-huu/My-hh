...
void MangPhuTang(float[][50], int, int);
void MangPhuGiam(float[][50], int, int);
...
int main()
{
	...
	int t = m * n;
	XuLy(a, n, t);
	cout << "\nMa tran sau khi sap duong tang, am giam, 0 giu nguyen :\nCach 1: ";
	Xuat(a, m, n);
...
void XuLy(float a[][50], int n, int t)
{
	...
	if (a[(t - 1) / n][(t - 1) % n] > 0)
	{
		for (int k = 0; k <= t - 2; k++)
		{
			if (a[k / n][k % n] > a[(t - 1) / n][(t - 1) % n] && a[k / n][k % n] > 0)
			{
...
void MangPhuTang(float a[][50], int m, int n)
{
	float b[1000];
	int k = 0;
	for (int i = 0; i < m; i++)
...
void MangPhuGiam(float a[][50], int m, int n)
{
	float b[1000];
	int k = 0;
	for (int i = 0; i < m; i++)
